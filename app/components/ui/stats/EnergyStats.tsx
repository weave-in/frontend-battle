...
function StatItem({ title, unit, value, change, yearlyData, chartImage }: StatItemProps) {
  ...
  return (
    <div className="mb-8 sm:mb-12">
      <div className="mb-2">
        <h3 className="text-gray-700 font-medium mb-1 text-sm sm:text-base">{title}</h3>
        <div className="flex items-baseline">
          <span className="text-3xl sm:text-5xl font-light text-gray-800 mr-2">{value}</span>
          <span className="text-gray-600 text-sm sm:text-base">{unit}</span>
        </div>
      </div>
      
      <div className="flex items-center mb-4">
        <span className="text-gray-500 text-xs sm:text-sm mr-2">from {changeYear}</span>
        <div className={`flex items-center ${changeDirection === 'up' ? 'text-red-500' : 'text-green-500'}`}>
          ...
          <span className="ml-1 text-xs sm:text-sm">{Math.abs(changeValue)}%</span>
        </div>
      </div>
